...
void Thread_COLOR (void *argument); // thread function 

extern osThreadId_t tid_COLOR;                       // thread id

void configurar_sensor(void);
void leer_registro(void); 
void iniciar_registros_sensor(void);

//Valor de color de una medida estandarizada en RGB (0-255)
typedef struct{
	float red;
	float green;
	float blue;
...
extern osMessageQueueId_t mid_COLORQueue;
